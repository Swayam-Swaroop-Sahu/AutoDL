...
def load_tabular_data(file_path: str) -> pd.DataFrame:
    ...
    if df.shape[0] < 2:
        raise ValueError(f"Dataset has too few rows ({df.shape[0]}). Minimum 2 rows required (1 for training, 1 for validation).")

    if df.shape[1] < 2:
        raise ValueError(f"Dataset has too few columns ({df.shape[1]}). Minimum 2 columns required (at least 1 feature + 1 target).")
